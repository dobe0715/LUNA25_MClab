...
def train_multitask(train_csv_path, valid_csv_path, exp_save_root):
    ...
    for patient in unique_patients:
        ...
        if cumulative_samples + patient_samples <= 100:
            ...
            if abs(100 - cumulative_samples) > abs(100 - (cumulative_samples + patient_samples)):
                selected_patients.append(patient)
                cumulative_samples += patient_samples
            break
    
    # If we still need more samples and there are remaining patients
    if cumulative_samples < 200 and len(selected_patients) < len(unique_patients):
    ...
    logging.info(f"New valid samples: {len(new_valid_df)} (target was 100)")
